...
string
string_view(string s, i32 index, i32 len) {
        assert(ISIN(0, index, s.len) && "Out of bounds");
        assert(ISIN(0, len, s.len - index) && "Out of bounds");
        return (string){.data = s.data + index, len};
}
